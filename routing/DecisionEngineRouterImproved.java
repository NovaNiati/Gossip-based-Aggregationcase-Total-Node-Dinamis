 ...
 * 
 * A DecisionEngineRouterImproved maintains a List of Tuple<Message, Connection> in 
 * support of a call to ActiveRouter.tryMessagesForConnected() in 
 * DecisionEngineRouterImproved.update(). Since update() is called so frequently, we'd 
 ...
 * RoutingDecisionEngineImproved object. The DecisionEngine Interface defines methods 
 * that enact computation and return decisions as follows:
 * 
 * <ul>
 *   <li>In createNewMessage(), a call to RoutingDecisionEngineImproved.newMessage() is 
 ...
 *   RoutingDecisionEngineImproved object. Also, on connection up events, this first
 ...
 *   OLD or DELIVERED, call to RoutingDecisionEngineImproved.shouldDeleteOldMessage() is
 ...
 *   RoutingDecisionEngineImproved.isFinalDest() to determine if the receiving (this) 
 *   host is an intended recipient of the Message. Next, a call to 
 *   RoutingDecisionEngineImproved.shouldSaveReceivedMessage() is made to determine if
